// ─────────────────────────────────────────────
//  KhetiBahi – RecordPaymentModal
//
//  A tiny, focused modal for the common follow-up
//  action: "the buyer just paid some/all of what
//  they owed." Faster than reopening the full
//  edit-sale form just to bump one number.
// ─────────────────────────────────────────────
...
import { formatNPR, amountDueFor } from "@/app/utils/helpers";

interface RecordPaymentModalProps {
  income: Income;
  onConfirm: (extraPaid: number) => void;
  onCancel: () => void;
}

export default function RecordPaymentModal({ income, onConfirm, onCancel }: RecordPaymentModalProps) {
  const due = amountDueFor(income);
      ...
      <div className="bg-surface rounded-3xl shadow-lift w-full max-w-sm">
        ...
        <div className="p-6 space-y-4">
          <p className="text-sm text-ink-muted">
            <span className="font-semibold text-ink">{income.buyer}</span> owes{" "}
            <span className="font-semibold text-accent tabular-nums">{formatNPR(due)}</span> for the{" "}
            {income.crop.toLowerCase()} sale on {income.date}.
          </p>

          <div>
            <label className="block text-sm font-medium text-ink-muted mb-1">Amount received now</label>
